...
export const handleCreateCrew = async (req, res, next) => {
    ...
    try {
        ...
        throw new InvalidInputValueError("올바른 형태의 크루 정보를 입력해주세요. (JSON 파싱 에러)", req.body)
    }
    try {
        req.body.applicationForm = JSON.parse(req.body.applicationForm);
    } catch (error) {
        throw new InvalidInputValueError("올바른 형태의 신청서 질문들을 입력해주세요. (JSON 파싱 에러)", req.body)
    }
    ...
    if (req.file === undefined || validImageExtensions.every((extension) => req.file.originalname.split(".").at(-1) !== extension)) {
        throw new InvalidInputValueError("올바른 크루 배너 이미지를 등록 해 주세요.", req.body);
    }
    // ✅ 유효성 검사 (크루 명)
    if (req.body.crewInfo.name === undefined || req.body.crewInfo.name === "" || typeof req.body.crewInfo.name !== "string") {
        throw new InvalidInputValueError("올바른 크루명을 입력해 주세요.", req.body);
    }
    // ✅ 유효성 검사 (크루 소개글)
    if (req.body.crewInfo.description === undefined || req.body.crewInfo.description === "" || typeof req.body.crewInfo.description !== "string") {
        throw new InvalidInputValueError("올바른 크루 소개글을 입력해 주세요.", req.body);
    }
    // ✅ 유효성 검사 (최대 모집 인원)
    if (req.body.crewInfo.maxCapacity === undefined || typeof req.body.crewInfo.maxCapacity !== "number") {
        throw new InvalidInputValueError("올바른 최대 모집 인원을 선택 해 주세요.", req.body);
    }
    // ✅ 유효성 검사 (카테고리)
    if (req.body.crewInfo.category === undefined || typeof req.body.crewInfo.category !== "number") {
        throw new InvalidInputValueError("올바른 카테고리를 선택 해 주세요.", req.body);
    }
    // ✅ 유효성 검사 (활동/액티비티)
    if (
        ...
        !req.body.crewInfo.activities.every((activity) => typeof activity === "number")
    ) {
    ...
    // ✅ 유효성 검사 (지역)
    if (req.body.crewInfo.region === undefined || typeof req.body.crewInfo.region !== "number") {
        throw new InvalidInputValueError("올바른 지역을 선택해주세요.", req.body);
    }
    // ✅ 유효성 검사 (연령대)
    if (req.body.crewInfo.age === undefined ||
    ...
    // ✅ 유효성 검사 (성별)
    if (req.body.crewInfo.gender === undefined ||
    ...
    if (req.body.applicationForm.questions.length === 0) {
        throw new InvalidInputValueError("최소 한개이상의 질문을 입력해 주세요.", req.body);
    }
    // ✅ 유효성 검사 (지원서 질문 세부 내용)
    const isQuestionsValid = req.body.applicationForm.questions.every((q) => {
        ...
        // 질문 타입 유효성 검사
        if (q.type === undefined || typeof q.type !== 'number' || q.type < 0 || q.type > 1) return false;
        // 질문 타입이 체크박스 형인 경우
        if (q.type === 0) {
            // 선택지 유효성 검사
            if (q.choices === undefined || q.choices.length === 0) return false;
            // 모든 선택지의 유효성(타입) 검사
            if (!q.choices.every((choice) => typeof choice === "string")) return false;
        }
        // 기타 항목 표시에 대한 유효성 검사
        if (q.etc === undefined || typeof q.etc !== "number" || q.etc < 0 || q.etc > 1) return false;
        // 필수 사항 여부에 대한 유효성 검사
        if (q.required === undefined || typeof q.required !== "number" || q.required < 0 || q.required > 1) return false;
        return true;
    })
    if (!isQuestionsValid) {
        throw new InvalidInputValueError("올바른 형식에 맞춰 질문을 작성 해 주세요.", req.body);
    }
